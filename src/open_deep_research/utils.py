...
async def tavily_search_async(search_queries, max_results: int = 5, topic: str = "general", include_raw_content: bool = True):
    ...
    """
    print(f"--- [tavily_search_async] Initializing Tavily client... ---") # DEBUG
    tavily_async_client = AsyncTavilyClient()
    search_tasks = []
    print(f"--- [tavily_search_async] Creating search tasks for queries: {search_queries} ---") # DEBUG
    for query in search_queries:
            print(f"--- [tavily_search_async] Creating task for query: '{query}' ---") # DEBUG
            search_tasks.append(
    ...
    print(f"--- [tavily_search_async] Starting asyncio.gather for {len(search_tasks)} tasks... ---") # DEBUG
    search_docs = await asyncio.gather(*search_tasks)
    print(f"--- [tavily_search_async] asyncio.gather completed. Received {len(search_docs)} results. ---") # DEBUG
    return search_docs
